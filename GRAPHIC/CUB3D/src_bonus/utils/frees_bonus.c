...
/*   Updated: 2023/08/17 15:36:27 by kramjatt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	delete_wall(t_vars *struc)
{
	...
	i = -1;
	while (++i < struc->button.nb_alloc)
		mlx_delete_texture(struc->button.textures[i]);
	if (struc->button.nb_alloc)
		free_arr_uint(struc->button.texture_tab);
	i = -1;
	while (++i < struc->weapons.nb_alloc)
		mlx_delete_texture(struc->weapons.textures[i]);
	i = -1;
	while (++i < struc->wall.nb_alloc)
		mlx_delete_texture(struc->wall.textures[i]);
	if (struc->wall.nb_alloc)
		free_arr_uint(struc->wall.texture_tab);
	i = -1;
	if (struc->door.nb_alloc)
	{
		free_arr_uint(struc->door.texture_tab);
		mlx_delete_texture(struc->door.textures);
	}
}
...
void	on_window_closed(t_vars *struc)
{
	...
	mlx_delete_image(struc->mlx_ptr, struc->img.weapon_ptr[0]);
	mlx_delete_image(struc->mlx_ptr, struc->img.weapon_ptr[1]);
	mlx_delete_image(struc->mlx_ptr, struc->img.weapon_ptr[2]);
	mlx_delete_image(struc->mlx_ptr, struc->img.weapon_ptr[3]);
	mlx_delete_image(struc->mlx_ptr, struc->img.weapon_ptr[4]);
	delete_wall(struc);
